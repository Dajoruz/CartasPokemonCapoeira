...
function endMatch() {
	...
	if (matchEndReason === "HUYO LA RATA") {
		var reason = ["TU OPONENTE", "TU"][+(socket.id !== matchWinner)] + " DEJASTE LA PARTIDA";
		labels["rematch"].disabled = true;
		labels["rematch"].clickable = false;
	} else {
		var reason = ["TU OPONENTE TIENE", "TU TIENES"][+(socket.id === matchWinner)] + " FULL PAPA";
		labels["rematch"].clickable = true;
	}

	labels["result"].text = ["QUE PERDEDOR!", "QUE CRACK SOS!"][+(socket.id === matchWinner)];
